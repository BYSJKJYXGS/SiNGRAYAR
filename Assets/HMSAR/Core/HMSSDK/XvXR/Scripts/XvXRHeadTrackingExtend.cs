...
public class XvXRHeadTrackingExtend : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (!HMS.Engine.XvXRManager.SDK.IsVRMode && startTouch && Input.GetTouch(0).phase == TouchPhase.Moved)
		{
		...
		{
			isFristLoad = false;
			SDK_OnVrModeChangeDelegate(HMS.Engine.XvXRManager.SDK.IsVRMode);
		}
	...
	void Start ()
	{
		...
		HMS.Engine.XvXRManager.SDK.ReCenterOnClick += SDK_ReCenterOnClick;
		HMS.Engine.XvXRManager.SDK.OnVrModeChangeDelegate += SDK_OnVrModeChangeDelegate;

		HMS.Engine.XvXRManager.SDK.ReCenter();
	...
	{
		if (HMS.Engine.XvXRManager.SDK.IsVRMode)
		{
			HMS.Engine.XvXRManager.SDK.ReCenter();
		}
		else
